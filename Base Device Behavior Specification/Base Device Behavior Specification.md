...
## 11.1 集中式 commissioning 的建议

### 11.1.1 集中式 commissioning 概述

集中式 commissioning 是一种允许固定或移动节点 commissioning（确定应用联系并创建绑定）其他节点（在同一网络上）的方法。这也可以称为 Gateway、Tool 或 S-Mode commissioning。

这可以是一个节点（如网关、中央控制器或通常连接到图形用户界面的 commissioning 工具）。此节点能够配置网络中其他节点上的绑定和报告。它也可以是从一个固定的预加载配置中自动 commissions 网络上其他节点的节点。

网络中任何具有此功能的节点都被定义为 Commissioning 主管（CD）。

![Figure 16 – Principle of centralized commissioning with a commissioning director](./pic/f16.jpg)

### 11.1.2 设备发现的建议

为了 commission 节点，CD 需要发现网络中的设备。下面列出了发现网络中所有节点的推荐方法。

#### 11.1.2.1 新节点加入

新节点通过广播 **Device\_annce** ZDO 命令宣布加入网络。然后，CD 可以使用 ZDO 发现服务来知道网络中的节点、绑定表服务以管理绑定表，并且如果需要，还可以使用 **groups** 簇的命令来管理分组表。

#### 11.1.2.2 现有网络中的节点

当 CD 加入现有网络时，它需要发现网络中已有的节点。CD 可以（MAY）在成功加入网络或某些用户刺激时立即启动此过程。此外，CD 可以（MAY）定期发现网络上的节点，以便及时了解任何变化。

CD 有多种方法可以发现网络中的节点，但建议（RECOMMENDED）CD 使用 **Mgmt\_Lqi\_req** ZDO 命令。下面列出了使用 **Mgmt\_Lqi\_req**（而不是 **IEEE\_addr\_req** 或 **NWK\_addr\_req**）的好处：

* ZigBee 协调器、ZigBee 路由器、ZigBee终端设备的 ZigBee 逻辑设备类型信息
* Rx_On_when_Idle 信息
* 有关父子关系的信息

CD 执行设备发现后，可以（MAY）执行进一步的 commission 动作，例如设置绑定或配置报告。

#### 11.1.2.3 与终端设备建立通信

本节是建议 CD 与终端设备通信的占位符，在有更好的理解用例时将会添加。
